...
int get_student(int fd, int id, student_t *s)
{
    ...
    int studentRead = read(fd, s, STUDENT_RECORD_SIZE);
    if (studentRead != STUDENT_RECORD_SIZE) {
        return ERR_DB_FILE;
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    if (memcmp(&s, &EMPTY_STUDENT_RECORD, sizeof(s)) != 0) {
    ...
    int writeStudent = write(fd, &s, STUDENT_RECORD_SIZE);
    if (writeStudent < 0) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
int del_student(int fd, int id)
{
    // TODO
    student_t s;
    int findStudent = get_student(fd, id, &s);
    if (findStudent == ERR_DB_FILE) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    ...
    int deleteStudent = write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
    if (deleteStudent < 0) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
